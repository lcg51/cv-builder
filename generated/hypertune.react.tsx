	...
	return (
		...
		>
			<HypertuneHydrator dehydratedState={dehydratedState}>
				<HypertuneRootProvider rootArgs={rootArgs}>{children}</HypertuneRootProvider>
			</HypertuneHydrator>
		</HypertuneSourceProvider>
	);
}
...
export function HypertuneSourceProvider({
	...
	const [stateHash, setStateHash] = React.useState(hypertuneSource.getStateHash());
	...
	return <HypertuneSourceContext.Provider value={value}>{children}</HypertuneSourceContext.Provider>;
}
...
export function HypertuneRootProvider({
	...
	return <HypertuneRootContext.Provider value={hypertuneRoot}>{children}</HypertuneRootContext.Provider>;
}
...
export function HypertuneHydrator({
	...
	if (rootArgs) {
		return <HypertuneRootProvider rootArgs={rootArgs}>{children}</HypertuneRootProvider>;
	}

	return children;
}

export function HypertuneClientLogger({ flagPaths }: { flagPaths: hypertune.FlagPaths[] }): null {
